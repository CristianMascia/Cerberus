...
#   NPARALLEL        slot di inferenza paralleli del server, -np/--parallel
#                    (default: 20, uno per ogni prompt di prompts.txt, cosi'
#                    tutte le richieste concorrenti inviate da query_models.py
#                    vengono servite davvero in parallelo e non messe in coda).
#                    Il contesto (-c in models.conf) viene diviso tra gli slot,
#                    quindi ogni slot ha una finestra di contesto pari a
#                    ctx/NPARALLEL.
...
for i in "${!NAMES[@]}"; do
    ...
    "$SINGULARITY" run --nv \
        ...
        -m "$model" \
        --host "$HOST" --port "$port" \
        -ngl "$NGL" -c "$ctx" --parallel "$NPARALLEL" --jinja \
